    ...
    try:
        ...
        if request.method=='POST':
            ...
            if user_otp==de_otp['server_otp']:
                cursor=mydb.cursor() # it is mysql cursor created using mysqldb connection object
...
def viewallnotes():
    ...
    try:
        ...
        stored_allnotesdata=cursor.fetchall() #[(1,'python','2025-0-23'),(2,'mysql','2026-09-23')]
...
def viewnotes(nid):
    ...
    try:
        ...
        stored_notesdata=cursor.fetchone() #(1,'python','easy to learn','2025-0-23')
...
def fileupload():
    if not session.get('user'):
        flash('pls login to access dahboard features')
